...
class NaiveModelThread(threading.Thread):
    def __init__(self, main_model, main_model_tokenizer, assist_model_tokenizer, information_dict, result_save_dir,
                 ...
                 device, device_compute, early_stop_string_list=None):
    ...
    def run(self) -> None:
        ...
        additional_kwargs = {
            ...
            "assist_model_probability_inverse_transfer_matrix_list": self.assist_model_probability_inverse_transfer_matrix_list,
        ...
        # 创建对象,创建logits_processor，从其他子进程拿到logits
        ...
        # target_model = logits_processor_instance.target_assist_model
        ...
        # model_answer, prediction = answer_extract(text, split_key_before_list,
        #                                           split_key_behind_list)
        # print(self.information_dict['question'])
        # print(prediction.strip())
        # model_answer_dict = {'prediction': prediction.strip(), 'main_model_input': main_model_input, 'all': text,
        #                      'model_answer': model_answer,}

        # result_file_path = os.path.join(self.result_save_dir,
        #                                 f'naive_model_output.jsonl')
